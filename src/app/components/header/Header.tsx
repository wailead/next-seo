    ...
    <SectionLayout>
      <header className="w-screen flex justify-between px-[0.65rem] bg-white-default h-[2.6rem] fixed top-0 left-0 right-0 z-30 items-center border-b-[1px]">
        <CustomImage
          src="/images/logo.png"
          alt="와이리(PC) 로고"
          w="primary:w-[5rem] w-[15vw]"
          h="primary:h-[2.2rem] h-[15vw]"
        />
        <div className="flex items-center primary:gap-[1.43rem] gap-[3.0524vw]">
          {isInfluencerPage && (
            <p className="font-pretendard font-normal primary:text-[0.78rem] text-[1.8vw] leading-[1.17rem] text-gray-300">
              수많은 캠페인에 신청해보세요.
            </p>
          )}
          {isAdvertiserPage && (
            <div className="flex justify-between items-center">
              <Link
                className="font-pretendard font-normal primary:text-[0.78rem] text-[1.8vw] text-gray-300 primary:px-[1.063rem] px-[2.4vw] h-[2.58rem] flex text-center items-center hover:bg-[#f6fbff] active:bg-[#cce7fd] transition-all duration-300 ease-out"
            ...
            {isInfluencerPage && (
              <>
                <LinkButton
                  ...
                  target="_blank"
                  textSize="primary:text-[0.65rem] text-[1.442vw]"
                  px="primary:px-[1.04rem] px-[2.9vw]"
